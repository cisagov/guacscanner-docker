...
```console
docker run cisagov/example:0.0.1-rc.11
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      example:
        image: cisagov/example:0.0.1-rc.11
        volumes:
          - type: bind
            source: <your_log_dir>
            target: /var/log
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/example:0.0.1-rc.11
    ```
...
```console
docker build \
  --build-arg VERSION=0.0.1-rc.11 \
  --tag cisagov/example:0.0.1-rc.11 \
